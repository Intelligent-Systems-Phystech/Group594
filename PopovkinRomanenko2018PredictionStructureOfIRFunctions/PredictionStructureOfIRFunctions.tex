...
$$\text{tf}(w, d, \textbf{C}) = freq(w, d) \cdot \log\left(1 + \frac{\text{size}_{\text{avg}}}{\text{size}(d)}\right)$$
...
$$f^* = \argmax_{f\in{\cal T}} {\left( \text{MAP}(f, \textbf{C}, \textbf{Q}) - P(f)\right)}$$где~$P(f)$~--- штрафная функция, ограничивающая структурную сложность суперпозиции~$f$.
...
$$f^* = \argmax_{W \in \mathbb{R}^m} \left(\text{MAP}(\sum_i W_i * f_i^*, \textbf{C}, \textbf{Q}) - \sum_i P(f_i^*)\right) $$

Оптимизацию весов $W$ будем производить при помощи поиска перебора. 
